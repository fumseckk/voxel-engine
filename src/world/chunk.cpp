...
bool Chunk::player_sees_face(Camera &camera, Direction dir)
{
...
Block Chunk::operator[](ivec3 p)
{
  assert(in_range(p));
  return blocks[ivec3_to_index(p)];
}

ivec3 Chunk::retrieve_chunk_coords(ivec3 p)
{
  // division rounded down for consistency in the negatives
  return floor((vec3)p / vec3(CHUNKS_SIZE));
}

bool Chunk::chunk_exists(ivec3 chunk_coords, unordered_map<ivec3, Chunk> &chunks)
{
  return chunks.find(chunk_coords) != chunks.end();
}

std::optional<Block> Chunk::get_world_block(ivec3 world_pos, unordered_map<ivec3, Chunk> &chunks)
{
...
void Chunk::set_face_at_coords(vec3 coords, Direction dir, BlockType type)
{
