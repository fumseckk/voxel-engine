...
BiomeType WorldGenerator::biome_from_noise(float noiseValue)
{
  ...
  if (mappedValue < 0.10f)
    return DESERT; // 10% chance
  if (mappedValue < 0.40f)
    return PLAINS; // 30% chance
  if (mappedValue < 0.70f)
    return FOREST; // 30% chance
  if (mappedValue < 0.85f)
    return MOUNTAINS; // 15% chance
...
int WorldGenerator::get_height(int x, int z, glm::ivec3 origin)
{
  ...
  int naturalTerrainHeight =
      std::min(WORLD_HEIGHT - 1, (int)((float)WORLD_HEIGHT * blendedHeight + 1));
